...
try {
    ...
    $result = $conn->query("SHOW DATABASES LIKE '{$safeDbName}'");
    if ($result && $result->num_rows === 0) {
        $conn->query("CREATE DATABASE `{$safeDbName}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    }
...
");
    ...
    while ($conn->more_results() && $conn->next_result()) {
        // Flush all CREATE TABLE results.
    }
} catch (Throwable $e) {
    error_log('[Saraswati DB] Table creation failed: ' . $e->getMessage());
    http_response_code(503);
    die(json_encode(['ok' => false, 'message' => 'Database table setup error. Please contact support.']));
}
...
try {
    ...
    error_log('[Saraswati DB] Seed/maintenance failed: ' . $e->getMessage());
    http_response_code(503);
    die(json_encode(['ok' => false, 'message' => 'Database seed error. Please contact support.']));
}

echo json_encode(['ok' => true, 'message' => 'Database initialized successfully.']);
?>
